...
export function useFhevm(parameters: {
  ...
  const _mockChainsRef = useRef<Record<number, string> | undefined>(initialMockChains as Record<number, string> | undefined);
  ...
  const refresh = useCallback(() => {
    ...
    _setError(undefined);
    _setStatus("idle");
    if (provider !== undefined) _setProviderChanged((prev) => prev + 1);
  }, [provider, chainId]);
